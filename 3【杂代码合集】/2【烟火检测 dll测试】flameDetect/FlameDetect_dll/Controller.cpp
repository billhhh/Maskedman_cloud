...
void Controller::flameDetect_close(){
	...
	string detectInfo = "";//全部检测信息叠加
	string detectInfo_single = "";//单次检测信息
	int count = 1; //帧计数，用于隔多少帧取一次
	int writeCount = 0;//同一秒只警告一次
	int flashCount =1;//相邻帧只显示一次
	int h,m,s;//时分秒
	double ratio;
	...
	while (1){
		...
		if(ifFlame_close(frame,mark,blockwNum,blockhNum)){ //检测到烟火
			...
			detectInfo_single = "";
			temp.str("");
			detectInfo_single += "在视频时间：";
			temp<<h;
			if(h/10 == 0)
				detectInfo_single += "0" ;
			detectInfo_single += temp.str() + ":";
			temp.str("");
			temp<<m;
			if(m/10 == 0)
				detectInfo_single += "0";
			...
			if(s/10 == 0)
				detectInfo_single += "0";
			detectInfo_single += temp.str()+ "  检测到烟火!\n";
			seq++;

			detectInfo += detectInfo_single;//单次信息叠加

			/*if(strlen(detectInfo.c_str())>=MAXSIZE){
				//cout<<detectInfo<<endl;
				strcpy(this->pAI_id->eventInfo , "");
				detectInfo = "";
			}*/
			strcat(this->pAI_id->eventInfo ,detectInfo_single.c_str());
			//strcpy(this->pAI_id->eventInfo , detectInfo.c_str());//写入烟火信息 小心字串越界！
			}//if writeCount
		...
		/*cout<<ncount<<"\t"<<this->pAI_id->percent<<endl;
		imshow("flame",this->pAI_id->curFrame_out);
...
// 		return;
	...
	{	
		...
		if(avg_scalar[0]>0 && avg_scalar[0]<18){ //0<饱和度<18，烟雾，报警，边缘红色
			...
			if(writeCount >= fps/skip){ //1s内只警告一次
				writeCount = 0;
			stringstream temp;
			filename = "/" + videoName + "_";
			s = ncount / fps +1;
			...
			detectInfo += detectInfo_single;//叠加
			/*if(strlen(detectInfo.c_str())>=MAXSIZE){
				//cout<<detectInfo<<endl;
				strcpy(this->pAI_id->eventInfo , "");
				detectInfo = "";
